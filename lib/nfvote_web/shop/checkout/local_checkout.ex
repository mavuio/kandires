defmodule KandiresWeb.Shop.LocalCheckout do
  @moduledoc false
  ...
  @all_steps ~w(welcome rgbericht neuwahlen antrag1 antrag2 review confirm finished)

  def check_if_redirect(conn) do
    conn
  end

  def get_cart_basepath(_params) do
    KandiresWeb.Router.Helpers.cart_path(KandiresWeb.Endpoint, :step)
  end

  def redirect_to_default_step(conn, params) do
    conn
    |> Phoenix.Controller.redirect(to: default_step_link(conn, params))
    |> Plug.Conn.halt()
  end

  def default_step_link(conn, params) do
    KandiresWeb.Router.Helpers.checkout_step_path(
      conn,
      :step,
      default_step(params)
    )
  ...
  def get_next_step_link(context, current_step) when is_map(context) do
    ...
    if next_step do
      KandiresWeb.Router.Helpers.checkout_step_path(
        KandiresWeb.Endpoint,
        :step,
        # KandiresWeb.FrontendHelpers.lang_from_params(context),
        next_step
      )
    else
  ...
  def get_cart_link(_context) do
    # lang = KandiresWeb.FrontendHelpers.lang_from_params(context)
  ...
  def extract_useful_vars_from_context(context) do
    ...
    %{lang: lang}
    |> Map.merge(context[:checkout_record] || %{})
    |> Map.merge(changes)
  ...
  def create_ordercart(cart, _lang, orderinfo) when is_map(cart) do
    cart
    |> update_in(
      [:items],
      fn _items ->
        create_pseudo_cart_items_from_orderinfo(orderinfo)
      end
    )
  end

  def create_pseudo_cart_items_from_orderinfo(orderinfo) do
    orderinfo
    |> Map.to_list()
    |> Enum.filter(fn {key, _} -> String.starts_with?("#{key}", "pkt_") end)
    |> Enum.map(fn {key, val} ->
      %{
        key: key,
        title: t("de", "points.#{key}"),
        result: val,
        type: "votetotal",
        price: "10",
        amount: 1
