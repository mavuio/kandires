defmodule KandiresWeb.Live.AuthHelper do
  require Logger

  # breaks with pow >1.0.18
  ...
  def mount_user(socket, pid, session, config) do
    ...
    |> case do
      ...
      {:ok, session_id} ->
        case credentials_by_session_id(session_id) do
          {user, meta} ->
            socket = socket |> assign(current_user: user, current_user_meta: meta)
    ...
    |> set_vid(session)
    |> IO.inspect(label: "mwuits-debug 2020-03-27_12:42 PPPPO")
  end

  defp set_vid(socket, session) do
    # similar code in to plugs/default_params.ex

    vid =
      case socket.assigns[:current_user] do
        %{id: id} -> id
        _ -> session["anon_userid"]
      end
